...
RSpec.describe Flat, type: :model do
  ...
  context 'validations' do
    it 'is not valid without user' do
      flat = Flat.create({ name: Faker::Restaurant.name, latitude: Faker::Address.latitude,
    ...
    it 'is not valid when latitude and longitude is not numeric' do
      user = create(:user)
      flat = user.flats.create({ name: Faker::Restaurant.name, latitude: Faker::Address.street_name,
                                 longitude: Faker::Address.street_name })
      expect(flat).to_not be_valid
    end
  end
end
