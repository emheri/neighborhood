...
RSpec.describe Neighborhood, type: :model do
  ...
  context 'validations' do
    it 'is not valid without a name' do
      neighborhood = Neighborhood.create({ name: nil, latitude: Faker::Address.latitude,
                                           longitude: Faker::Address.longitude, radius: 1000 })
      expect(neighborhood).to_not be_valid
      expect(neighborhood).to have(1).error_on(:name)
    end

    it 'is not valid without a latitude' do
      neighborhood = Neighborhood.create({ name: Faker::Name.name, latitude: nil, longitude: Faker::Address.longitude,
                                           radius: 1000 })
      expect(neighborhood).to_not be_valid
    ...
    it 'is not valid without a longitude' do
      neighborhood = Neighborhood.create({ name: Faker::Name.name, latitude: Faker::Address.latitude, longitude: nil,
                                           radius: 1000 })
      expect(neighborhood).to_not be_valid
    ...
    it 'is not valid without radius' do
      neighborhood = Neighborhood.create({ name: Faker::Name.name, latitude: Faker::Address.latitude,
                                           longitude: Faker::Address.longitude, radius: nil })
      expect(neighborhood).to_not be_valid
      expect(neighborhood).to have(2).error_on(:radius)
    end

    it 'is not valid when latitude and longitude is not numeric' do
      neighborhood = Neighborhood.create({ name: Faker::Restaurant.name, latitude: Faker::Address.street_name,
                                           longitude: Faker::Address.street_name, radius: 1000 })
      expect(neighborhood).to_not be_valid
    ...
    it 'is not valid when radius is not numeric' do
      neighborhood = Neighborhood.create({ name: Faker::Name.name, latitude: Faker::Address.latitude,
                                           longitude: Faker::Address.longitude, radius: 'radius' })
      expect(neighborhood).to_not be_valid
